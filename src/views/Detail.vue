<template>
  <div class="detail">
    ...
    <van-goods-action fixed>
      <van-goods-action-icon :to="{name: 'Shopbag'}" icon="bag" text="购物袋" :badge="bagCount == 0 ? '' : bagCount" :color="bagCount > 0 ? '#1A73E8' : '#646566'" />
      <van-goods-action-icon icon="like" :text="isLike ? '已收藏' : '未收藏'" :color="isLike ? '#1A73E8' : '#646566'" @click="toggleLikeProduct" />
      <van-goods-action-button text="加入购物袋" color="#6A91EC" @click="addShopbag()" />
      <van-goods-action-button  text="立即购买" color="#1A73E8" @click="buy(true)" class="buy" />
    </van-goods-action>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //立即购买(后台判断购物袋如果存在该商品，则累加数量，然后跳转到订单结算页面，
      // 否则先将该商品加入购物袋，再跳转到订单结算页面)
      buy(isBuy) {
